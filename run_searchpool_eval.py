...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate model performance on similarity search tasks"
    )
    ...
    if not (
        dataset_base.with_suffix(".pickle").exists()
        or Path(str(dataset_base) + "-graphs.pickle").exists()
    ):
    ...
    logger.info(
        f"Initializing evaluator with {len(args.pool_sizes)} pool sizes: {args.pool_sizes}"
    )
